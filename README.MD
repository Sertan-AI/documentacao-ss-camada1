...
> [!NOTE]
> A origem da geometria refere-se exclusivamente ao processo interno de geração da Camada 1. Métricas como NDVI podem ser utilizadas internamente durante esse processo, porém não são persistidas no banco de dados nem fazem parte do contrato de dados no V0.
...
| **Evidência no Banco de Dados** | **Evidência na Interface** |
|:-------------------------------:|:--------------------------:|
| <img src="static/demo/evid_demo_banco.png" alt="Banco de Dados" style="height: 300px; width: auto;"> | <img src="static/demo/evid_demo_interface.png" alt="Interface" style="height: 300px; width: auto;"> |
...
> [!IMPORTANT]
> **Registros Temporais no MVP (V0)**  
> Todos os registros temporais do MVP (V0) são gerados no servidor, nunca pelo cliente.  
> O timezone adotado é UTC, garantindo consistência temporal entre sessões e ambientes.  
> Os seguintes campos possuem semântica distinta:  
> ● `observed_at`: momento em que o técnico realizou a observação em campo  
> ● `created_at`: momento em que o sistema persistiu o registro no banco  
> Dentro de uma mesma sessão, os timestamps são monotonicamente não regressivos, garantindo ordenação temporal consistente.
...
> [!NOTE]
> A Camada 1 pode consumir dados externos (ex.: NDVI) exclusivamente para fins de visualização descritiva, sem persistência, sem garantia e fora do contrato de dados.
